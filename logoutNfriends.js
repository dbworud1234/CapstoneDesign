        ...
        success : function(data){
          ...
          if(!!friend){ ///// 로컬 유저랑 친구 맺은 것도 추가해 줘야 함...
            for(let i in friend){
              ...
              printF+=`<div class="frlist" style="background-color:#eceff6; height:60px;"><img src="${fr.profile_thumbnail_image}" style="border-radius:8px; width:60px; height:60px; float:left;" > <span style="display:block;">${fr.profile_nickname}</span><div onclick="event.cancelBubble=true;"><button id="${fr.id}"onclick="onclickfollowBtn(${fr.id})" name="uf">${msg}</button></div></div>`;
            }
